...
async function init() {
 const response = await fetch('content.json?v=4'); if (!response.ok) throw new Error('Content unavailable'); const data = await response.json();
...
function showStill(index) { currentIndex = (index + currentStills.length) % currentStills.length; const still = currentStills[currentIndex]; const image = lightbox.querySelector('img'); image.src = still.src; image.alt = still.alt; lightbox.querySelector('#still-counter').textContent = (currentIndex + 1) + ' / ' + currentStills.length; }
function openGallery(stills, index) { if (!lightbox) return; currentStills = stills; galleryOpener = document.activeElement; showStill(index); lightbox.showModal(); document.body.style.overflow = 'hidden'; }
if (lightbox) { lightbox.querySelector('.lightbox-close').addEventListener('click', () => lightbox.close()); lightbox.querySelector('.lightbox-prev').addEventListener('click', () => showStill(currentIndex - 1)); lightbox.querySelector('.lightbox-next').addEventListener('click', () => showStill(currentIndex + 1)); lightbox.addEventListener('keydown', event => { if (event.key === 'ArrowRight') { event.preventDefault(); showStill(currentIndex + 1); } if (event.key === 'ArrowLeft') { event.preventDefault(); showStill(currentIndex - 1); } }); lightbox.addEventListener('close', () => { document.body.style.overflow = ''; galleryOpener?.focus(); }); lightbox.addEventListener('click', event => { if (event.target === lightbox) { const rect = lightbox.getBoundingClientRect(); if (event.clientX < rect.left || event.clientX > rect.right || event.clientY < rect.top || event.clientY > rect.bottom) lightbox.close(); } }); }
